...
本示例介绍如何使用阿里云Java SDK调用SLB的 DescribeLoadBalancerHTTPSListenerAttribute 接口，查询HTTPS监听配置。
 ...
 * 查询HTTPS监听配置
 *
 * @see https://help.aliyun.com/api/slb/DescribeLoadBalancerHTTPSListenerAttribute.html
 *
 */
public class DescribeLoadBalancerHTTPSListenerAttribute {
    ...
    public static void main(String[] args) {
        ...
        DescribeLoadBalancerHTTPSListenerAttributeResponse response;
        // 设置请求参数
        request.setRegionId("cn-beijing");
        // 负载均衡实例ID
        request.setLoadBalancerId("lb-2zeb619c6tveo8u7b5ey4");
        // 负载均衡实例前端使用的端口
        request.setListenerPort(433);
        ...
        try {
            ...
            System.out.println(String.format(""
                    ...
                    + "ServerCertificateId: %s, CACertificateId: %s" + "\n"
                    + "Gzip: %s, TLSCipherPolicy: %s",
                    ...
                    response.getServerCertificateId(), response.getCACertificateId(),
                    response.getGzip(), response.getTLSCipherPolicy()
            ));
